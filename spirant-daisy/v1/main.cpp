...
// --- System Bus validation sink (phase 3 only) ------------------------------
// This phase wires the complete communication stack -- boot GetAll, MSG
// polling, GetChanged, CRC, timeouts, GetAll fallback -- to a Pod LED rather
// than to any synthesis parameter. The v0 synth and its local controls are
// deliberately untouched, so that "the comms are broken" and "parameter
// application is broken" can never be confused for one another, and so that
// the audio-non-disturbance requirement is proven before the DSP changes.
//
// The parameter engine replaces this with the patch staging in phase 4.
//
// Both values are single-word and shared between the main loop (writer) and
// the audio callback (reader) -- the same atomicity assumption v0 already
// makes for note_frequency and breath_level on the Cortex-M7.

// Normalised 0..1 position of the most recently received parameter, whatever
// it was. Any encoder on any page therefore moves LED1.
float led_target = 0.f;

// System::GetNow() timestamp of the last failed transaction; LED2 flashes red
// for kCommErrorFlashMs afterwards. Zero means "never failed".
uint32_t comm_error_at_ms = 0;

constexpr uint32_t kCommErrorFlashMs = 120;
                    ...
                    size_t                                size)
{
    ...
    // --- Phase 3 LED roles (v0's breath/reverb indicators are suspended) ---
    // LED1: white at the brightness of the most recently received parameter.
    // This is the validation sink -- turning any encoder on the Pico must
    // visibly move it, while notes are playing.
    //
    // Note that LED1 no longer shows the chorus mono/stereo state, so the
    // button1 toggle stays audible but becomes invisible this phase. It is
    // removed entirely in phase 4.
    hw.led1.Set(led_target, led_target, led_target);

    // LED2: red flash on any failed transaction or CRC mismatch, off
    // otherwise. With no logic analyser on the bench this is the only
    // visibility into bus faults, so it is a deliverable, not decoration.
    const bool comm_error_recent
        = comm_error_at_ms != 0
          && (System::GetNow() - comm_error_at_ms) < kCommErrorFlashMs;
    hw.led2.Set(comm_error_recent ? 1.f : 0.f, 0.f, 0.f);
    hw.UpdateLeds();
...
// Map a received record onto the LED brightness, using the parameter's own
// range from the table. Called once per frame with the LAST record, not once
// per record: a boot GetAll carries 19 of them and strobing the LED through
// all of them buys nothing.
void apply_record_to_led(const Record& record)
{
    const ParamRange& range = kParamTable[record.global_idx];
    const float       span  = (float)(range.max - range.min);
    if(span <= 0.f)
        return;
    led_target = fclamp((float)(record.value - range.min) / span, 0.f, 1.f);
}
...
int main()
{
    ...
    // Audio starts BEFORE the parameter sync this phase, which is the reverse
    // of the eventual ordering (protocol section 3.1). Deliberate: the v0
    // synth does not need the Pico's values, and starting audio first lets the
    // milestone demonstrate that even the boot-time retry storm -- a GetAll
    // every 10 ms, indefinitely, if the Pico is absent -- cannot disturb
    // running audio. Phase 4 restores audio-after-sync, which is about
    // correct parameter state rather than audio safety.
    hw.StartAudio(audio_callback);
    hw.midi.StartReceive();

    // Onboard LED lit until the first CRC-verified frame arrives.
    hw.seed.SetLed(true);

    Record  records[kMaxRecords];
    uint8_t count = system_bus_boot_sync(records);
    if(count > 0)
        apply_record_to_led(records[count - 1]);

    hw.seed.SetLed(false);

    uint32_t last_sync_ms = System::GetNow();
    ...
    for(;;)
    {
        ...
        // Time-gated sync tier. I2C transactions are blocking (~2.4 ms for the
        // fixed 97-byte frame) but run here in the main loop, never in the
        // audio callback, so audio is unaffected; MIDI bytes buffer in the
        // UART FIFO meanwhile. Idle cost is one GPIO read per 33 ms.
        if(System::GetNow() - last_sync_ms >= kSyncIntervalMs)
        {
            last_sync_ms = System::GetNow();

            bool          failed  = false;
            const uint8_t applied = system_bus_sync(records, &failed);

            if(applied > 0)
                apply_record_to_led(records[applied - 1]);

            if(failed)
                comm_error_at_ms = System::GetNow();
        }
    }
}
